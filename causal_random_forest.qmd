...
Cross-fitting attempts to limit these biases in DML by fitting the model to one subset of data, and then obtaining the plug-in estimates for the other subset. This ensures that the prediction for any given data point is not influenced by the model fit on that same data point. This probably looks a lot like cross-validationm but it is used in the fitting stage, not evaluation
