...
2. Estimate the propensity equation, and residuals based on this:

$$
E(W_i|X_i=x) = \hat{W}_i \sim \beta_W X_i; \\  W_i'= W_i - \hat{W}_i
$$

3.  Estimate the causal effect by regressing the residuals:
...
![Frisch-Waugh-Lovell workflow](images/fwl.png)
...
This was further extended by Chernozhukov in 2018. Essentially, to get to the final stage model, all we need are plug-in estimates of the outcome conditional on $X_i$, and the treatment assignment conditional on $X_i$. Chernozhukov demonstrated that, under certain conditions, these plug-in estimates can be obtained from any predictive model, including standard machine learning approaches (e.g. SVM, RF, XGBoost). 

The conditions to be met involve dealing with two types of bias:

- Regularization bias
- Overfitting bias

These biases can be addressed through orthoganalization (the FWL type approach), cross-fitting and the use of honest trees. 

### Cross-ftting

Cross-fitting attempts to limit these biases in DML by splitting the data into folds. The model is fit to one fold of data, and then plug-in estimates are obtained for the other fold This ensures that the prediction for any given data point is not influenced by the model fit on that same data point. This probably looks a lot like cross-validationm but it is used in the fitting stage, not evaluation. Cross-fitting improves estimation of standard errors and enables valid inference

![Cross-fitting workflow](images/crossfitting.png)

### Honest trees

This basically means that any tree has to use a different set of points for constructing the splits and for predicting the labels. This is distinguished from cross-fitting by applying to individual trees (not the whole forest model, as in cross-fitting). 

In general, data are split into folds to estimate the models, then honest trees are built within the forest. The honest approach maps particularly well to the RF algorithm, as the bootstrapping of samples ensures there are always a split into in-bag and out-of-bag samples for each tree (and a different split for each tree).

## Non-constant treatment effects

For In the original equation, $\tau$ is assumed to be a constant factor across all individuals. To relax this, they use the idea of subgroups within the data set, each of which has it's own regression, giving a value of $\tau$ for each group ($\tau$ here is just the coefficient of a linear model based on the centered outcome and treatment). The equation now becomes:
...
As an aside: Athey, Tibshirani and Wager extend this idea to generalized random forests. In these, the splits are formed simply by maximizing the "heterogeneity in some arbitrary parameter given as the solution to [an] equation". The **grf** package includes functions for probability, quantile and survival forests (for example).
...
ii. Using the IB set, for each feature ($X_j$): a. Iterate across values of $X_j$ to partition the OOB data into two sets ($L$ and $R$) b. Test for imbalance (hyperparameter: 25-75% is maximum imbalance by default): skip if imbalance is greater than this c. Fit the following model in each of the two partitions:
